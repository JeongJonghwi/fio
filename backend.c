	...
	while ((td->o.read_iolog_file && !flist_empty(&td->io_log_list)) ||
		...
		if (((td_write(td) && io_u->ddir == DDIR_WRITE) || io_u->ddir == DDIR_TRIM) &&
			...
			if (!log_io_piece(td, io_u)) {
				/*
				 * Overlap with in-flight IO. Wait for at least one
				 * completion to make forward progress.
				 */
				dprint(FD_IO, "Overlap detected, waiting for completion\n");
				invalidate_inflight(td, io_u);
				clear_io_u(td, io_u);
				ret = FIO_Q_BUSY;
				goto reap;
...
static uint64_t do_dry_run(struct thread_data *td)
{
	...
	while ((td->o.read_iolog_file && !flist_empty(&td->io_log_list)) ||
		...
		if (td_write(td) && io_u->ddir == DDIR_WRITE &&
		    ...
		    !td->o.experimental_verify) {
			/*
			 * In dry run, if overlap is detected, just skip.
			 * No need to serialize since no real I/O happens.
			 */
...
static void *thread_main(void *data)
{
	...
	if (o->verify_table_id > 0) {
		...
		/* If this is a write job, increment the active write jobs counter */
	...
	while (keep_running(td)) {
		 ...
		 * Use atomic exchange to ensure only the first job does verify.
		 */
		if (td->shared_verify_table) {
			int expected = 0;
